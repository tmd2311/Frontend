...
export const CustomerManagement: React.FC = () => {
  ...
  const goToCustomerDetail = (id: string) => {
    router.push(`/admin-app/customers/detail/${id}`);
  };
          ...
          {loading ? (
            ...
            <table className="w-full min-w-full">
              <thead className="bg-gray-50 sticky top-0">
                <tr>
                  <th className="px-6 py-4 text-left text-xs font-semibold">Người Dùng</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold">Liên hệ</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold">Tổng đơn</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold">Tổng chi tiêu</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold">Đơn gần nhất</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold">Trạng thái</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold">Thao tác</th>
                </tr>
              </thead>
              <tbody>
                {filtered.map((c) => (
                  <tr key={c.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 flex">
                      <button
                        ...
                        onClick={() => goToCustomerDetail(c.id)}
                      >
                        <Eye size={16} />
                      </button>
